...
class DbHelper
{
    static function updateColumnValue($table_name, $col,$col_val,$change_col,$change_val)
    {
        ...
        DB::connection('sqlsrv')->table($table_name)
            ->where($where)
            ->update($table_data);
    ...
    static function getColumnValue($table_name, $col,$col_val,$return_col)
    {
        ...
        $qry = DB::connection('sqlsrv')->table($table_name)
            ->select($return_col)
            ->where($where);
    ...
    static function insertRecord($table_name, $table_data, $user_id=null)
    {
        ...
        try {
            ...
            DB::connection('sqlsrv')->transaction(function () use ($table_name, $table_data, $user_id, &$res) {
                $table_data['created_on'] = Carbon::now();
                $table_data['created_by'] = $user_id;
                $record_id = DB::connection('sqlsrv')->table($table_name)->insertGetId($table_data);
                $res = array(
                    'success' => true,
                    'record_id' => $record_id,
    ...
    {
        $record = DB::connection('sqlsrv')->table($table)->where($where)->first();
        return $record;
    }

    static function getSingleRecordColValue($table, $where, $col)
    {
        $val = DB::connection('sqlsrv')->table($table)->where($where)->value($col);
        return $val;
    }
    ...
    {
        $results = DB::connection('sqlsrv')->select(DB::connection('sqlsrv')->raw($qry));
        return $results;
    }
